...
import { Phone, Video, MoreVertical, Smile, Paperclip, Mic, Send, Loader2, X, CornerUpRight, Pin, Trash2 } from "lucide-react";
...
export default function ChatWindow({ selectedChat, onBack }) {
  ...
  const MessageBubble = ({ msg }) => {
    ...
    // 🔥 CORRECTION 1 : Vérifier correctement l'ID utilisateur
// 🔥 Détermination robuste de l'expéditeur
const currentUserId =
  user?._id || user?.id || user?.userId;

// ton backend renvoie Id_sender (string), pas senderId
const rawSender =
  msg.senderId || msg.sender || msg.Id_sender || msg.Id_User || msg.userId;

// si c'est un objet (populate), on prend _id, sinon la string
...
const { reactions, addReaction, removeReaction } = useReactions(msg._id);
    ...
    const isMatch = searchTerm && msg.content?.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    // 🔥 CORRECTION 2 : Gérer l'épinglage
    ...
    // 🔥 CORRECTION 3 : Gérer la suppression
      ...
      <div className={`flex ${fromMe ? "justify-end" : "justify-start"} group`}>
        <div className="flex flex-col max-w-[85%] relative">
          ...
          <div className="relative">
            <div
              className={`${bubbleClasses(fromMe)} ${isMatch ? "ring-2 ring-blue-400" : ""} cursor-pointer`}
              style={{ 
            ...
            >
              {/* Pin indicator */}
              {msg.isPinned && (
                <Pin size={12} className="absolute -top-1 -right-1 text-myYellow" />
              )}
              ...
              {msg.typeMessage === 'image' && (
                <img src={msg.fileUrl} alt="image" className="max-w-full rounded mt-1" style={{ maxHeight: '300px' }} />
              )}
              
              {msg.typeMessage === 'video' && (
                <video src={msg.fileUrl} controls className="max-w-full rounded mt-1" style={{ maxHeight: '300px' }} />
              )}
              
              {/* 🔥 CORRECTION 4 : Audio avec durée */}
{msg.typeMessage === "audio" && (
  <AudioMessage src={msg.content || msg.fileUrl} />
              ...
              {msg.typeMessage === 'file' && (
                <a href={msg.fileUrl} download className="flex items-center gap-2 underline">
  ...
  <div
    className={`message-menu absolute ${fromMe ? 'right-0' : 'left-0'} top-full mt-1 bg-white dark:bg-neutral-800 rounded-lg shadow-xl z-50 py-1 min-w-[150px]`}
  >                <button
                  onClick={() => {
  ...
  <div
    className={`reaction-picker absolute ${fromMe ? 'right-0' : 'left-0'} top-full mt-1 bg-white dark:bg-neutral-800 rounded-full shadow-xl z-50 px-2 py-1 flex gap-1`}
    ...
    {reactions.map((reaction) => (
      ...
      >
        ...
        {reaction.id_user && (
          ...
          {/* Heure et statut */}
          <div className={`text-[8px] mt-1 flex items-center gap-1 ${fromMe ? 'justify-end' : 'justify-start'} text-gray-700 dark:text-gray-300`}>
    ...
    <div className="relative flex flex-col w-full h-full bg-myWhite dark:bg-neutral-900 text-myBlack dark:text-white transition-colors duration-300 min-w-[150px]" style={themeStyle}>
      ...
      <header className="flex items-center justify-between px-2 py-2 border-b border-gray-300 dark:border-gray-700 backdrop-blur-sm bg-white/20 dark:bg-black/20 z-20">
        <div className="flex items-center gap-2 min-w-0">
          <button onClick={onBack} className="md:hidden mr-2 text-xl">←</button>
          <img src={conversationAvatar} alt="avatar" className="w-8 h-8 rounded-full object-cover" />
          <div className="truncate">
            <div className="text-sm font-semibold truncate">{conversationName}</div>
            {/* 🔥 CORRECTION 5 : Typing indicator visible */}
            <div className="text-xs truncate text-gray-700 dark:text-gray-300">
              {isTyping && typingUsers.length > 0
                ? <span className="text-green-500 font-medium">{t("chat.typing") || "En train d'écrire"}...</span>
        ...
        <div className="flex items-center gap-2">
          <Phone size={16} className="text-gray-600 dark:text-gray-300 cursor-pointer" />
          ...
          <button onClick={() => setIsOptionsOpen(true)}>
            <MoreVertical size={16} className="text-gray-600 dark:text-gray-300 cursor-pointer" />
          </button>
        </div>
      </header>
      ...
      {/* 🔥 CORRECTION 6 : Messages dans le bon ordre (plus récent en bas) */}
<main
  ref={messagesContainerRef}
  className="flex-1 overflow-y-auto px-2 py-2 space-y-3 relative z-10"
  onClick={(e) => {
    // si on clique dans le menu ou le picker, on ne ferme pas
...
>
        {messages.map((msg, i) => {
          const showDate = i === 0 || messages[i - 1].createdAt?.split('T')[0] !== msg.createdAt?.split('T')[0];
        ...
        {isRecording && (
         ...
         <div className="flex items-center gap-2">
          <div className="flex items-center gap-1 px-2 py-2 rounded-xl flex-1 bg-myGray4 dark:bg-[#2E2F2F] backdrop-blur-md">
            <Smile size={18} className="text-gray-700 dark:text-gray-300 cursor-pointer" />
            <Paperclip 
              size={18} 
          ...
          <button 
            ...
            onClick={inputText.trim() === "" ? handleMicClick : handleSendMessage}
          >
            {inputText.trim() === "" ? (
              <Mic size={18} className={`text-gray-700 dark:text-gray-300 ${isRecording ? "animate-pulse text-red-500" : ""}`} />
            ) : (
              <Send size={18} />
            )}
          </button>
      ...
      {openSearch && (
        <>
          <div className="fixed inset-0 z-40" onClick={() => setOpenSearch(false)}></div>
          <motion.div
