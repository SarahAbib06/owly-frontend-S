...
const Login = () => {
  ...
  // État pour la vérification OTP inactivité
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Cas 2 : Nécessite vérification OTP (inactivité)
        ...
        setRequiresOtp(true);
        setOtpToken(response.debug_token);
        navigate("/OtpPage", { 
          state: { 
        ...
        <form className="w-full mx-auto" onSubmit={handleSubmit}>
          ...
          <p className="text-left text-sm mt-4 mb-4">
            <button
              ...
              className="text-xs text-myBlack cursor-pointer mt-2 mb-8 hover:text-myGray2 transition-colors duration-300"
            >
          ...
          <p className="text-[12px] w-full sm:text-left text-center md:text-center mt-3 text-myBlack">
            {t("login.noAccount")}{" "}
